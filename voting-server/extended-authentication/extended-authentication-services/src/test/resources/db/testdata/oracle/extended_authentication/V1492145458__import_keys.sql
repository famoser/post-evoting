Insert into EXTENDED_AUTHENTICATION.EA_PLATFORM_CA_CERTIFICATES (ID,PLATFORM_NAME,CERTIFICATE_NAME,CERTIFICATE_CONTENT) values (EA_PLATFORM_CERTIFICATES_SEQ.NEXTVAL,'SwissPost','dev Root CA',TO_CLOB(q'[-----BEGIN CERTIFICATE-----

zQ+gpluOYWwHZRkVr]')
|| TO_CLOB(q'[hG50NL8PfOVrWi8MTtQ35JejZDwU8LJmmLKFDrwyfGxnFpD
]')
|| TO_CLOB(q'[95
BCRU1d44piHlxR/F3kQEw1m/vPIRwRbUl5cJNZeLys8c9Tp4esjXF+F/kYf/E7tA
BtuikvwSoTSvgPHFK6QxXMbjEsRWlX8Z6pVZf8PDKdu/pakyk6By43dXzkRm9nlR
HxgA/r+m0r4y9/QZOpRuiDEYGPEElW4VN3OaAaoJ/3FI+BE=
-----END CERTIFICATE-----
]'));