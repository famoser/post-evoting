...
public class ChoiceCodeResourceTest extends JerseyTest {

	public static final String URL_CHOICE_CODE_CONTEXT_DATA = ChoiceCodeResource.RESOURCE_PATH;
	...
	public void getStatusOfVotingCards() throws IOException {
		...
		reply.addProperty("choiceCodes", "areHere");
		...
		when(votingWorkflowVotingClient
				.getChoiceCodes(eq(ChoiceCodeResource.CHOICE_CODE_PATH), eq(TENANT_ID), eq(ELECTION_EVENT_ID), eq(VOTING_CARD_ID), eq(AUTH_TOKEN),
						eq(X_FORWARDER_VALUE), eq(TRACK_ID))).thenReturn(callMock);

		Response response = target(URL_CHOICE_CODE_CONTEXT_DATA).resolveTemplate("tenantId", TENANT_ID)
	...
	protected Application configure() {
		...
		sut = new ChoiceCodeResource(votingWorkflowVotingClient, trackIdGenerator);
