...
/**
 * Web service for handling data of the electoral authorities in the vote verification context (VV)
 */
@Stateless(name = "ag-vv-ElectoralDataResource")
@TransactionAttribute(TransactionAttributeType.NOT_SUPPORTED)
@Path(ElectoralDataVVResource.RESOURCE_PATH)
public class ElectoralDataVVResource {

	static final String SAVE_ELECTORAL_DATA = "tenant/{tenantId}/electionevent/{electionEventId}/electoralauthority/{electoralAuthorityId}/adminboard/{adminBoardId}";

	static final String RESOURCE_PATH = "/vv/electoraldata";
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(ElectoralDataVVResource.class);
	...
	@Inject
	ElectoralDataVVResource(VoteVerificationAdminClient voteVerificationAdminClient, TrackIdGenerator trackIdGenerator) {
	...
	public Response saveElectoralData(
		...
		try (ResponseBody responseBody = RetrofitConsumer.processResponse(voteVerificationAdminClient
				.saveElectoralData(ELECTORAL_DATA_PATH, tenantId, electionEventId, electoralAuthorityId, adminBoardId, xForwardedFor, trackingId,
						body))) {
			return Response.ok().build();
		} catch (RetrofitException rfE) {
			LOGGER.error("Error trying to save electoral data.", rfE);
